...
Future<void> exportToPdf(List<Map<String, dynamic>> data) async {
  ...
  if (!kIsWeb) {
    ...
    const String path = 'Edit_Responses.pdf';
    ...
    final anchor = html.AnchorElement(href: url)
      ..setAttribute('download', 'Edit_Responses.pdf')
      ..click();
...
Future<void> exportToExcel(List<Map<String, dynamic>> data) async {
  ...
  for (var message in data) {
    ...
    sheet.appendRow([
      ...
      TextCellValue(message['actual_answer']),
      TextCellValue(message['modified_answer']),
  ...
  if (!kIsWeb) {
    final String dir = (await getDownloadsDirectory())!.path;
    const String path = 'Edit_Responses.xlsx';
    final File file = File('$dir/$path');
    ...
    // Create anchor element
    final anchor = html.AnchorElement(href: url)
      ..setAttribute('download', 'Edit_Responses.xlsx')
      ..click();
...
Future<void> exportToWordDocument(List<Map<String, dynamic>> data) async {
  ...
  for (var message in data) {
    ...
    documentContent += '<p class="date">${DateFormat.yMMMMd('en_US').format(timestamp)}</p>'
        ...
        '<p class="answer">${message['actual_answer']}</p>'
        '<p class="modified_answer">${message['modified_answer']}</p>'
